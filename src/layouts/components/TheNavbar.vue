...
<div class="relative">
	<div class="vx-navbar-wrapper">
		<vs-navbar class="vx-navbar navbar-custom" :color="navbarColor" :class="classObj">
			...
			<!-- I18N -->
			<!-- <vs-dropdown vs-custom-content vs-trigger-click>
				<span class="cursor-pointer flex"><img class="h-4 w-5" :src="require(`@/assets/images/flags/${$i18n.locale}.png`)" :alt="$i18n.locale" /><span class="hidden sm:block ml-2">{{ getCurrentLocaleData.lang }}</span></span>
			...
			<!-- USER META -->
			<div class="the-navbar__user-meta flex items-end sm:ml-5 ml-2">
				<div class="text-right leading-tight hidden sm:block">
					<!-- <p class="font-semibold">John Doe</p>
					<small>Available</small> -->
				</div>
                <vs-row type="flex" vs-w="12" vs-justify="flex-end">
				<vs-dropdown vs-custom-content vs-trigger-click>
					<div class="con-img ml-3"><img :src="require('@/assets/images/influencer.jpg')" alt="" width="40" height="40" class="rounded-full shadow-md cursor-pointer block"></div>
					<vs-dropdown-menu>
						<ul style="min-width: 9rem">
							...
							<li class="flex py-2 px-4 cursor-pointer hover:bg-primary hover:text-white" @click="clicklogout"><feather-icon icon="LogOutIcon" svgClasses="w-4 h-4"></feather-icon> <span class="ml-2">Logout</span></li>
						</ul>
					</vs-dropdown-menu>
				</vs-dropdown>
                </vs-row>
...
export default {
    ...
    methods: {
        async logoutapi(tok) {
      try {
        const response = await service.userLogout(tok);
        console.log(response)
        console.log(response.data.data);
        this.$store.dispatch("removeAccessToken");
        console.log(this.$store.state.accessToken,'logout')
        // this.brandDetails = response.data.data;
      } catch (error) {
        ...
        async clicklogout() {
            ...
            // this.$router.push('/')
            this.logoutapi(this.$store.state.accessToken)
			this.$cookies.remove("auth");
			this.$router.push("/");
        },
        updateLocale(locale) {
